...
//        advanceUntilIdle()
    ...
    fun `deleteUser should delete user with correct id`() = runTest(timeout = 60.seconds) {
        ...
        val deletedUser = userDtoCollection.find(Filters.eq("_id", userTest.id)).firstOrNull()
    ...
    fun `getUserById returns user when found`() = runTest {
        ...
        userDtoCollection.find(Filters.eq("_id", userTest.id))
    ...
    fun `getUserByName throws exception when user not found`() {
        ...
        coEvery { collection.find(Filters.eq("_id", "User Name")) } returns mockk {
    ...
    fun `login returns user when credentials match`() = runTest {
        ...
        val filter = Filters.and(Filters.eq(USER_NAME, username), Filters.eq(PASSWORD, password))
    ...
    fun `login returns null when credentials do not match`() = runTest {
        ...
        val filter = Filters.and(Filters.eq("username", username), Filters.eq("password", password))
