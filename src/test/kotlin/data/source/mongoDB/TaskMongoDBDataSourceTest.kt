...
class TaskMongoDBDataSourceTest {
    ...
    fun `createTask should push task to the correct project`() = runTest {
        ...
        val filter = Filters.eq("_id", task2.projectId)
    ...
    fun `createTask throws exception on database error`() {
        ...
        coEvery { collection.updateOne(Filters.eq("_id", "false projectId"), Updates.push("tasks", task)) } throws Exception(
            "DB error"
        )
        runTest {
            // Act & Assert
    ...
    fun `editTask should update the matching task in the project`() = runTest {
        ...
        val filter = Filters.and(
            Filters.eq("_id", task.projectId),
            Filters.eq("tasks.id", task.id)
        )
